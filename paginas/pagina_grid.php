...
session_start();
extract($_REQUEST);
$seletabla2 = $_REQUEST[seletabla2];
...
include ("../../persistencia/clsPostgre.php");
  ...
  {  
	  ...
	  if($_REQUEST[seletabla] == 'tg018_usuarios')
	  {  
	     $seletabla2 = obtener_clave($_REQUEST[seletabla2]);
	  }
...
echo "<form action='pagina_grid.php' method='post'>";
$loj = '';
...
if($datos["column_name"] != $campoclave)
{
  ...
  if($datos["column_name"] == 'in_estatus') $nu_estatus = $w;
...
echo "<tr>";
if($_REQUEST[seletabla] != 'tg004_auditoria')
...
for($h=0; $h < count($head); $h++)
{
  ...
  echo "<TH align='center' class='Tabla-Elemento-Encabezado'><a href=pagina_grid.php?order=".$labelorig[$h]."&seletabla=".$_REQUEST[seletabla]."&ordtype=".$ordtype.">".$cabe."</a></TH> ";
...
{   $datos=$ld_reg->fetch(PDO::FETCH_BOTH);
    ...
    ?>
    <td class=<?php echo $style ?> ><img src="../imagenes/ico_visualizar_on.png" name="boton_consultar" width="15" height="16" border="0" id="boton_consultar" style="cursor:pointer" title="Permite consultar el registro seleccionado" onClick="location.href = 'pagina_consultaregistro.php?seletabla='+parent.document.getElementById('seletabla').value+'&campoclave='+<?php echo "'".$campoclave."'" ?>+'&valorclave='+<?php echo "'".$valorclave."'" ?>"  value="Actualizar"></a></td>

<?php
if($_REQUEST[seletabla] != 'tg004_auditoria')
{ ?>
    <td class=<?php echo $style ?> ><img src="../imagenes/ico_actualizar_on.png" name="boton_actualizar" width="15" height="16" border="0" id="boton_actualizar" style="cursor:pointer" title="Permite actualizar el registro seleccionado" onClick="location.href = 'pagina_modificaregistro.php?seletabla='+parent.document.getElementById('seletabla').value+'&campoclave='+<?php echo "'".$campoclave."'" ?>+'&valorclave='+<?php echo "'".$valorclave."'" ?>"  value="Actualizar"></a></td>
<?php } ?>	
    <?php 
     $styleaux = '';
     while ($i <= $cuantas) 
     {
	   ...
	   if($i > 0)
       { 
	      if(($i == $nu_estatus+1) && ($nu_estatus > 0))
		  {
		     if($datos[$i] == 1)
			 { echo "<td class=".$style."><img src='../imagenes/accept.png'></td> "; }
			 else
			 { echo "<td class=".$style."><img src='../imagenes/stop.png'></td> ";}
		  }else{
...
<tr>
<?php if ($page > 1) { ?>
<td><a href="pagina_grid.php?page=<?php echo $page - 1 ?>&seletabla=<?php echo $_REQUEST[seletabla]?>&ordtype=<?php echo $_REQUEST[ordtype]?>&order=<?php echo $_REQUEST[order]?>"><img src='../imagenes/ico_atras_on.png'></a></td>
<?php } ?>
<?php
  global $pagerange;
...
<?php } else { ?>
<td><a href="pagina_grid.php?page=<?php echo $j ?>&seletabla=<?php echo $_REQUEST[seletabla]?>&ordtype=<?php echo $_REQUEST[ordtype]?>&order=<?php echo $_REQUEST[order]?>"><?php echo nupagina($j).' ' ?></a></td>
<?php } } } else { ?>
<td><a href="pagina_grid.php?page=<?php echo $startpage ?>&seletabla=<?php echo $_REQUEST[seletabla]?>&ordtype=<?php echo $_REQUEST[ordtype]?>&order=<?php echo $_REQUEST[order]?>"><?php echo nupagina($startpage) ."..." .nupagina($count) ?></a></td>
<?php } } } ?>
<?php if ($page < $pagecount) { ?>
<td>&nbsp;<a href="pagina_grid.php?page=<?php echo $page + 1 ?>&seletabla=<?php echo $_REQUEST[seletabla]?>&ordtype=<?php echo $_REQUEST[ordtype]?>&order=<?php echo $_REQUEST[order]?>"><img src='../imagenes/ico_adelante_on.png'></a>&nbsp;</td>
<?php } ?>
</tr>
</table>
<?php } ?>
